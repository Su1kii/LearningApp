...
export const api = {
  ...
  getEnrollmentStatus: (courseId: number) =>
    axios.get<{ enrolled: boolean }>(`${API_URL}/courses/${courseId}/enrollment-status`),
  createCourse: (data: { title: string; description: string; subject: string }) =>
  ...
  getSubmissions: (assignmentId: number) =>
    axios.get<Submission[]>(`${API_URL}/submissions/assignment/${assignmentId}`),
